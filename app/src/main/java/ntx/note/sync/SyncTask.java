...
import ntx.note.data.TemporaryDirectory;
import ntx.note.sync.HttpPostBase.Response;
import ntx.note.sync.SyncData.SyncItem;
...
public class SyncTask extends AsyncTask<SyncData, SyncTask.Progress, Void> {
	...
	private NoteAccount account = null;
	...
	private void pullNotebook(SyncItem item) throws SyncException {
		...
		Bookshelf.getInstance().importBookDirectory(tmp, item.getUuid());
