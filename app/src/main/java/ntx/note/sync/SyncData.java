 ...
 * @author vbraun
 */
public class SyncData implements Iterable<SyncData.SyncItem> {
    ...
    protected final NoteAccount account;
    protected String sessionToken;

    public SyncData(SharedPreferences syncPreferences, NoteAccount account) {
    ...
    public static class SyncItemComparator implements Comparator<SyncItem> {
        ...
        public int compare(SyncItem lhs, SyncItem rhs) {
            State lhsState = lhs.getState();
            State rhsState = rhs.getState();
            // Conflict is on top
            if ((!lhsState.equals(State.CONFLICT)) && (rhsState.equals(State.CONFLICT))) return +1;
            if ((lhsState.equals(State.CONFLICT)) && (!rhsState.equals(State.CONFLICT))) return -1;

            // Local-only is next to bottom
            if ((lhsState.equals(State.LOCAL_ONLY)) && (!rhsState.equals(State.LOCAL_ONLY)))
                return +1;
            if ((!lhsState.equals(State.LOCAL_ONLY)) && (rhsState.equals(State.LOCAL_ONLY)))
                return -1;
    ...
    protected void addLocal(Book book) {
    ...
    protected NoteAccount getAccount() {
        return account;
    }
