...
public abstract class TouchHandlerABC {
    ...
    public float getScaledPenThickness() {
        if (ToolboxConfiguration.getInstance().getCurrentTool() == Tool.FOUNTAINPEN)
            return Stroke.getScaledPenThickness(view.getPage().transformation,
     ...
     * @return Boolean. True if the event should be considered as coming from the
     * pen
     */
    ...
    /**
     * Called from the HandWriterView's onDraw() method while the touch interaction
     * is in progress
     *
     * @param canvas The canvas to draw to
     * @param bitmap The bitmap of the currently shown part of the page. Usually you'll
     *               want to call canvas.drawBitmap(bitmap, 0, 0, null) to paint it
     *               onto the canvas. But the touch handler may want to draw some user
     *               feedback in addition (or instead). For example, the pen touch
     *               handler draws a rough preview of the pen stroke.
     */
    protected abstract void draw(Canvas canvas, Bitmap bitmap);

    /**
     * Interrupt the current action, e.g. the user pressed the back key while
     * drawing a line.
     */
     ...
     */
    protected Transformation pinchZoomTransform(final Transformation transformation, float oldX1, float newX1,
                                                float oldX2, float newX2, float oldY1, float newY1, float oldY2, float newY2) {
     ...
     */
    protected float pinchZoomScaleFactor(float oldX1, float newX1, float oldX2, float newX2, float oldY1, float newY1,
                                         float oldY2, float newY2) {
     ...
     * <p>
     * This method just paints the region in the bitmap to the canvas where it would
     * be after a pinch-zoom gesture. This is much faster than actually redrawing
     * the bitmap with the new transformation, so we use this shortcut during the
     * gesture. Only when the gesture is finished do we redraw the bitmap with the
     * new zoom and offset.
     */
    protected void drawPinchZoomPreview(Canvas canvas, Bitmap bitmap, float oldX1, float newX1, float oldX2,
                                        float newX2, float oldY1, float newY1, float oldY2, float newY2) {
